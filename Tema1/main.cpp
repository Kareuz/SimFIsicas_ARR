...
int main() {
   ...
   groundBodyDef.position.Set(0.0f, -10.0f);  //Se define la posición del cuerpo
    ...
    fixtureDef.density = 1.0f; //Característica de densidad
    fixtureDef.friction = 0.3f; //Característica de fricción

    body->CreateFixture(&fixtureDef); //Se le aplican las características anteriores al "body"

    float timeStep = 1.0f / 60.0f; //Determinamos cada cuanto se realiza una acción, interación o lo que sea
